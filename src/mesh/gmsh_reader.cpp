...
//    std::vector<unsigned int> gmsh_l2h = dealii::Utilities::invert_permutation(gmsh_h2l);                       //PHYSICAL INTERPRETATION BACK TO GMSH
        ...
        for (const auto &cell : high_order_grid->dof_handler_grid.active_cell_iterators()) {
            ...
            if (cell->is_locally_owned()) {                                               //IF CELL IS OWNED BY THIS PROCESSOR
                ...
                auto high_order_flip_id_rotated = high_order_vertices_id_lexico;
                auto high_order_x_id_rotated = high_order_vertices_id_lexico;
                auto high_order_y_id_rotated = high_order_vertices_id_lexico;

                //Preallocate all indices
                pcout << "Rotate Z" << std::endl;
                rotate_indices<dim>(rotate_z90degree_3D, grid_order + 1, 'Z');
                pcout << "" << std::endl;
                pcout << "Rotate Y" << std::endl;
                rotate_indices<dim>(rotate_y90degree_3D, grid_order + 1, 'Y');
                pcout << "" << std::endl;
                pcout << "Rotate X" << std::endl;
                ...
                if (dim == 2) {
                    for (int zr = 0; zr < 4; ++zr) {                                                                            //FOR QUADS
                        ...
                        for (unsigned int i_vertex = 0;
                             i_vertex < cell->n_vertices(); ++i_vertex) {                              //ONLY VERTEX POINTS
                            ...
                            if (!found) {
                                std::cout << "Wrong cell... High order nodes do not match the cell's vertices | "
                    ...
                    for (int zrf = 0; zrf < 2; ++ zrf) {

                        pcout << "Begin 3D Rotation" << std::endl;

                        if (zrf == 1) {
                            pcout << "Normal 3D Rotation Failed. Initiating Flip Axis Rotation" << std::endl;
                        }
                        ...
                        for (int xr = 0; xr < 4; ++xr) {                                                                            //Rotate X
                            for (int zr = 0; zr < 4; ++zr) {                                                                        //Rotate Y
                                for (unsigned int zr3d = 0; zr3d < 4; ++zr3d) {                                                     //Rotate Z
                                    ...
                                    if (zrf == 1 && flip_counter == 0) {                                                                     //Only flip once
                                        for (unsigned int i = 0; i < high_order_vertices_id.size(); ++i) {
                                            high_order_flip_id_rotated[i] = high_order_vertices_id[rotate_flip_z90degree_3D[i]];
                                        }
                                    }

                                    for (unsigned int i = 0; i < high_order_vertices_id.size(); ++i) {

                                        if (zrf == 1 && flip_counter == 0) {
                                            high_order_vertices_id_rotated[i] = high_order_flip_id_rotated[rotate_z90degree_3D[i]];
